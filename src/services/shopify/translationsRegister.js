...
export default async function translationsRegister(productID,translations) {
  const mutation = gql`
...
// };
  ...
  try {
    ...
    const translations = res.translationsRegister.translations;
    if (translations) {
      return translations;
    }
    ...
    console.error(`❌ 更新翻譯錯誤：`, error.response?.errors || error.message);
    return null;
  }
}